...
const DataTable = class {
    ...
    render() {
        ...
        return (h(Host, null, h("table", null, h("thead", null, h("tr", null, this.columns.map(column => h("th", { onClick: (event) => column.sortable && this.sort(event, column), class: { "no-sort": !column.sortable } }, column.label)))), h("tbody", null, rows.map((row) => h("tr", null, this.columns.map((column) => {
...
const ListItemCategory = class {
    ...
    render() {
        return (h(Host, { onClick: () => this.categoryClickedHandler(this.category) }, h("img", { ref: (el) => this.image = el, src: this.category.iconUrl }), h("p", null, this.category.name)));
    }
...
const ListItemLocation = class {
     ...
     */
    render() {
        return (h(Host, { role: "listitem", onClick: () => this.locationClickedHandler(this.location) }, this.location.properties.imageURL ? this.renderIcon() : null, h("div", { class: "details" }, h("p", { class: "details-title" }, this.location.properties.name), h("mi-location-info", { ref: (el) => this.infoElement = el })), this.distance ? this.renderDistance() : null));
    }
    /**
...
const ScrollButtons = class {
    ...
    render() {
        return (h("div", { class: "scroll-buttons" }, h("button", { class: "mi-button mi-button--base btn btn-up", type: "button", disabled: true, "aria-label": "Scroll Up", ref: (el) => this.upButtonElement = el, onClick: () => this.updateScrollPosition(-this.scrollLength) }), h("button", { class: "mi-button mi-button--base btn btn-down", type: "button", "aria-label": "Scroll Down", ref: (el) => this.downButtonElement = el, onClick: () => this.updateScrollPosition(this.scrollLength) })));
    }
    static get watchers() { return {
...
const StepSwitcher = class {
    ...
    render() {
        return (h(Host, null, h("button", { type: "button", disabled: this.steps.length === 0 || this.stepIndex === 0, onClick: () => this.setStepIndex(this.stepIndex - 1) }, h("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", width: "24px", height: "24px" }, h("path", { d: "M0 0h24v24H0V0z", fill: "none" }), h("path", { d: "M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z" }))), h("div", { class: "steps" }, this.heading ? h("p", null, this.heading) : null, h("div", { class: "steps__dots" }, this.steps.map((item, index) => {
            const isActive = this.stepIndex === index ? true : false;
            return h("span", { class: { 'active': isActive } });
        }))), h("button", { type: "button", disabled: this.steps.length === 0 || this.stepIndex === this.steps.length - 1, onClick: () => this.setStepIndex(this.stepIndex + 1) }, h("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", width: "24px", height: "24px" }, h("path", { d: "M0 0h24v24H0V0z", fill: "none" }), h("path", { d: "M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6-1.41-1.41z" })))));
    }
...
const Tabs = class {
    ...
    render() {
        return (h(Host, null, h("nav", null, h("ul", null, this.tabs.map((tab, index) => {
